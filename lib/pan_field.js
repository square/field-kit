...
(function() {
  ...
  PanField = (function() {
    ...
    PanField.prototype.__defineGetter__('wordBreakIndexes', function() {
      var index, result, _i, _len, _ref;
      result = [0];
      _ref = this.formatter.constructor.GAP_INDEXES;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        index = _ref[_i];
        result.push(index + 1);
      }
      if (result[result.length - 1] !== this.value.length) {
        result.push(this.value.length);
      }
      return result;
    });
    ...
    PanField.prototype.nextWordBreakAfterIndex = function(index) {
      var indexes, result, wordBreakIndex, _i, _len;
      indexes = this.wordBreakIndexes.reverse();
      result = indexes[0];
      for (_i = 0, _len = indexes.length; _i < _len; _i++) {
        wordBreakIndex = indexes[_i];
        if (index < wordBreakIndex) {
    ...
    PanField.prototype.insertDigit = function(event) {
      if (this.hasSelection) {
        this.clearSelection();
      }
    ...
    PanField.prototype.__defineGetter__('caret', function() {
      var left, leftPadding, realCaret, rightPadding, valueCaret;
      realCaret = this.element.caret();
      left = this.text.substring(0, realCaret.start);
    ...
    PanField.prototype.__defineSetter__('caret', function(caret) {
      var i, nonWhitespaceCharacterCount, realCaret, text, _i, _ref;
      text = this.text;
      realCaret = {
        start: 0,
        end: 0
      ...
      for (i = _i = 0, _ref = text.length; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
        ...
        if (nonWhitespaceCharacterCount === caret.start) {
          realCaret.start = i;
        }
        if (nonWhitespaceCharacterCount === caret.end) {
          realCaret.end = i;
          break;
        }
        nonWhitespaceCharacterCount++;
      }
      if (realCaret.start !== realCaret.end && text[realCaret.end - 1] === GAP) {
        realCaret.end--;
      }
      return this.element.caret(realCaret);
    });
