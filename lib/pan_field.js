...
(function() {
  ...
  PanField = (function() {
    ...
    PanField.prototype.__defineGetter__('caret', function() {
      var caret, realCaret, text, textIndex, value, valueIndex;
      value = this.value;
      text = this.text;
      ...
      while (textIndex <= text.length) {
        if (text[textIndex] === value[valueIndex]) {
          if (realCaret.start === textIndex) {
            caret.start = valueIndex;
          }
          if (realCaret.end === textIndex) {
    ...
    PanField.prototype.__defineSetter__('caret', function(caret) {
      var max, min, realCaret, text, textIndex, value, valueIndex;
      value = this.value;
      text = this.text;
      realCaret = {
        start: 0,
      ...
      while (textIndex <= text.length) {
        if (text[textIndex] === value[valueIndex]) {
          if (caret.start === valueIndex) {
            realCaret.start = textIndex;
          }
          if (caret.end === valueIndex) {
            realCaret.end = textIndex;
          }
          textIndex++;
          valueIndex++;
        } else {
