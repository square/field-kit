...
module.exports = function() {
  test.describe('FieldKit.SocialSecurityNumberFormatter', function() {
    var input;
    test.beforeEach(function() {
      server.goTo('social_security_number_formatter.html');
    ...
    test.it('places dashes in the right places', function() {
      ...
      return helpers.getFieldKitValues()
        .then(function(values) {
          expect(values.raw).to.equal('123-45-6789');
        });
    });

    test.it('prevents extra digits from being entered', function() {
      helpers.setInput('123-45-6789|', input);
    ...
    test.it('backspaces words correctly', function() {
      helpers.setInput('123-45-|', input);

      input.sendKeys(Key.chord(Key.ALT, Key.BACK_SPACE));

      return helpers.getFieldKitValues()
        .then(function(values) {
          expect(values.raw).to.equal('123-');
        });
    });
  });
};
